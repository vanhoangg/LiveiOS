...
private let TAG_BUTTON_PROCESS = 9089
private let TAG_BUTTON_REJECT = 9090
private let TAG_BUTTON_FINISH = 9091
private let TAG_BUTTON_CONFIRM = 9092
...
class OrderConfirmController: BaseController {
    ...
    @objc func actionButton(_ sender: UIButton) {
        ...
        if sender.tag == TAG_BUTTON_CONFIRM {
            ...
            Server.shared.createOrder(id: order.id,
                                      ...
                                      userShipping: userS!.toDict(), {[weak self] (_, err) in
                ...
                if err == nil {
                    ...
                    let cancelAction: UIAlertAction = UIAlertAction(title: "ok".localized(), style: .cancel) { _ -> Void in
            ...
            Server.shared.changeStatusOrder(orderIds: [order.id],
                                            status: sta, {[weak self] (order, err) in
                                                ...
                                                if err != nil {
                                                    ...
                                                    let cancelAction: UIAlertAction = UIAlertAction(title: "ok".localized().uppercased(), style: .cancel) { _ -> Void in
                                                        // Just dismiss the action sheet
                                                    }
                                                    actionSheetController.addAction(cancelAction)
                                                    _self.present(actionSheetController, animated: true, completion: nil)
                                                    ...
                                                    let cancelAction: UIAlertAction = UIAlertAction(title: "continue".localized().capitalizingFirstLetter(), style: .cancel) { _ -> Void in
                                                        // Just dismiss the action sheet
                                                    }
                                                    let backAction: UIAlertAction = UIAlertAction(title: "close".localized().capitalizingFirstLetter(), style: .default) {[weak _self] _ -> Void in
                                                        // Just dismiss the action sheet
    ...
    private func loadLastShipping() {
        ...
        Server.shared.getShippings(from: user.id) {[weak self] (shippings, _) in
