...
class LiveStreamController: BaseController, LFLiveSessionDelegate, CreateStreamViewDelegate, TLPhotosPickerViewControllerDelegate, FilterViewDelegate, ListProductDelegate, ShopProductViewDelegate {
    ...
    func handlingSocket() {
        ...
        socket.on("update_private_quantity") {[weak self] data, _ in
            guard let _self = self else {
                return
            }
            if let data = data[0] as? [JSON] {
    ...
    func startCountVideoTimes() {
        ...
        timerVideoLive = Timer.scheduledTimer(withTimeInterval: 1, repeats: true) { (_) in
    ...
    func listernEvent() {
        ...
        categoryController.onTouchButton = {[weak self] _ in
            guard let _self = self else {return}
    ...
    func onUploadThumbClick() {
        ...
        viewController.didExceedMaximumNumberOfSelection = { [weak self] (_) in
    ...
    func didTappedStartLiveButton(_ button: UIButton) {
    ...
    func didTappedBeautyButton(_ button: UIButton) {
    ...
    func didTappeCameraaButton(_ button: UIButton) {
        let devicePositon = session.captureDevicePosition
        session.captureDevicePosition = (devicePositon == AVCaptureDevicePosition.back) ? AVCaptureDevicePosition.front : AVCaptureDevicePosition.back
    }
