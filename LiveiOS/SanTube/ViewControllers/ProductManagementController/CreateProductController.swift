...
class CreateProductController: BaseController, CreateProductViewDelegate, PickPreviousProductDelegate {
    ...
    override func viewDidLoad() {
        ...
        buttonProduct.imageEdgeInsets = UIEdgeInsets(top: 0, left: -20, bottom: 0, right: 0)
    ...
    func actionBack(_ sender: Any) {
		...
		let yes = UIAlertAction(title: "Yes".localized(), style: .default, handler: { [unowned self] (_) in
    ...
    func actionSaveProduct(_ sender: Any) {
        ...
        for view in listView {
            ...
            if product != nil {
				if (product?.price)! < Float(0.0) {
					...
					let ok = UIAlertAction(title: "OK", style: .cancel, handler: { [unowned self] (_) in
        ...
        if listData.count > 0 {
            ...
            Server.shared.createProducts(userId: user.id, streamId: self.streamID, products: listData.flatMap {$0.toDict()}) { [weak self] result, _ in
                if result != nil {
